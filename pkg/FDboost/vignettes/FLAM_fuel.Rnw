...
Load FDboost package and useful functions for plotting.
...
legend("topright", legend=c("data", "BS", "mean BS", "5, 95% BS"), 
       lty=c(5,1,1,2), col=c(4,8,1,2),
       lwd=c(2,1,2,2))

plot(mod, which=2, lwd=2, col="white", main="", lty=5, rug=FALSE, 
     ylab="", xlab="normalized wavelength", ylim=range(val$coefCV[[2]]$value) )
plotPredCoef(val, terms=FALSE, commonRange=TRUE, which=2, add=TRUE)
plot(mod, which=2, lwd=2, col=4, main="", lty=5, rug=FALSE, add=TRUE)
#dev.off()
@
\end{center}
\caption{Coefficient estimates for the effects of the two spectra. The gray lines
show the estimates in the 50 bootstrap folds, the black line gives the mean estimated coefficient function over the bootstrap folds, the dashed red lines point-wise 5\% and 95\% values, the dotdashed blue lines give the estimated coefficients for the model on the whole dataset. In each plot the zero line is marked by a dotted black line.}
\end{figure}


<<model-spec-plot2, echo=TRUE, fig = FALSE>>=
pdf("spec_valCoef.pdf")
par(mar=c(5, 3, 2, 1), cex.axis=1.5, cex.lab=1.5)
plot(mod, which=1, lwd=2, col="white", main="UV-VIS", lty=5, rug=FALSE,
...
plot(mod, which=2, lwd=2, col="white", main="NIR", lty=5, rug=FALSE, 
