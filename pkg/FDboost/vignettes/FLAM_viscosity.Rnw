...
\begin{figure}
\begin{center}
<<plot-data, echo=TRUE, fig = TRUE>>=
# pdf("vis.pdf")
# par(mfrow=c(1,1), mar=c(3, 3, 1, 2), cex=1.5)
mycol <- gray(seq(0, 0.8, l=4), alpha=0.8)
...
folds <- cvMa(ydim=modAll$ydim, weights=model.weights(modAll), 
              type="bootstrap", B=10)
cvmAll <- suppressWarnings(cvrisk(modAll, folds = folds, 
...
Do variable selection using stability selection.
...
set.seed(1911)
folds <- cvMa(ydim=mod1$ydim, type="bootstrap", B=10)
cvm1 <- suppressWarnings(cvrisk(mod1, folds = folds, 
                                grid=seq(10, 500, by=10), mc.cores=10))
...
<<plot-selected-model, echo = TRUE, fig=TRUE>>=
# pdf("visMod.pdf")
# par(mfrow=c(1,1), mar=c(3, 3, 1, 2), cex=1.5)
