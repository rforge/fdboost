...
\title{Viscosity: scalar-on-function regression}
...
\begin{figure}
\begin{center}
<<plot-bootstrap-model1a, echo = TRUE, fig=TRUE, height=6, width=10>>=
par(mfrow=c(1,2), mar=c(4,4,2,1), cex=1.5, cex.main=0.9)
...
@
\caption{The estimated coefficients for the four climatic regions are plotted with color coded regions (left panel).  The coefficient function for the functional effect of temperature is colored in red for positive values and in blue for negative values (right panel).}
\end{center}
\end{figure} 
...
<<plot-bootstrap-model1b, echo = TRUE, fig=TRUE, height=6, width=10>>=
par(mfrow=c(1,2), mar=c(4,4,2,1), cex=1.5, cex.main=0.9)
predRegion <- predict(mod3, which=1, 
                      newdata=list(region=factor(c("Arctic","Atlantic",
                                                   "Continental","Pacific")),
...
\begin{figure}
\begin{center}
<<plot-bootstrap-model2, echo = TRUE, fig=TRUE, heigth=10, width=10>>= 
