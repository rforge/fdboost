### changed code bkronecker.R of package mboost
### so that a base-learner can have a transformation matrix Z on the design matrix 
### (with Z depending on the design matrix), 
### and that same Z is used to transform a new design matrix in predict()
...
bl_lin_matrix <- function(blg, Xfun, args) {
    ...
    # <SB> add Z as further argument to change it in args 
    # in model fit: Z=NULL
    # in predict(): Z=transformation matrix, if necessary
    newX <- function(newdata = NULL, Z=NULL) {
        if (!is.null(newdata)) {
    ...
    X <- newX()
    K <- X$K     
    ### <SB> remember Z, so that it is in the environment of dpp()
    Z <- X$Z       
    X <- X$X
    c1 <- ncol(X$X1)
    c2 <- ncol(X$X2)
    ...
    dpp <- function(weights) {
        ...
        if (is.null(args$lambda)) {
            ...
            lambdadf <- mboost:::df2lambda(matrix(0, ncol = ncol(X$X1) + ncol(X$X2)),
        ...
        cf <- lapply(bm, function(x) x$model)
            if(!is.null(newdata)) {
                ...
                nm <- names(blg)
                newdata <- newdata[nm]  
                # <SB> use Z as further argument
                X <- newX(newdata, Z=Z)$X
            }
            ncfprod <- function(b)
...
"%O%" <- function(bl1, bl2) {
    ...
    Xfun <- function(mf, vary, args) { # <SB> args is a list containing Z      
      #browser()
      ...
      #ls(sys.frame(which = 1))
        ...
        ### <SB> return the transformation matrices Z1 and Z2 as well
        list(X = list(X1 = X1, X2 = X2), K = K, Z = list(Z1 = Z1, Z2 = Z2))
    }
